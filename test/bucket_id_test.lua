...
function g.test_bucket_id_sharding_func_body_with_error()
    g.schema.spaces.space.sharding_func = {
        body = 'function( end' -- Syntax is wrong intentionally.
    }
    local ok, err = ddl.set_schema(g.schema)
    t.assert_equals(err, nil)
    t.assert_equals(ok, true)
    ...
    local id, err = ddl.bucket_id('space', REFERENCE_KEY)
    t.assert_str_contains(err, "Body is incorrect in sharding_func for space (space)")
    t.assert_equals(id, nil)
end
...
function g.test_bucket_id_sharding_func_userdata_call()
    ...
    -- Create a userdata object and set sharding function as a __call method of
    -- userdata object.
...
function g.test_bucket_id_sharding_func_userdata_wo_call()
    local sharding_func_name = 'sharding_func_userdata_wo_call'
    ...
    local ok, err = ddl.set_schema(g.schema)
    t.assert_str_contains(err,
        "spaces[\"space\"].sharding_func: unknown sharding_func \"sharding_func_userdata_wo_call\"")
...
function g.test_bucket_id_sharding_func_cdata_call()
    ...
    -- Create a cdata object and set sharding function as a __call method of
    -- cdata object.
    ffi.cdef[[
        typedef struct {
            double x;
...
function g.test_bucket_id_sharding_func_cdata_wo_call()
    local sharding_func_name = 'sharding_func_cdata_wo_call'
    ...
    ffi.metatype('point_type', { })
    local point = ffi.new('point_type')
    t.assert_equals(type(point), 'cdata')
    ...
    local id, err = ddl.bucket_id('space', REFERENCE_KEY)
    t.assert_str_contains(err,
        "Failed to execute sharding function for space name (space)")
    t.assert_str_contains(err,
        "is not callable")
    t.assert_equals(id, nil)
