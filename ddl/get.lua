...
local function prepare_sharding_func_for_call(space_name, sharding_func_def)
    ...
    if type(sharding_func_def) == 'table' then
        if sharding_func_def.body== nil then
            return nil, string.format(
                "Body is empty in sharding_func for space (%s)", space_name)
        end
        sharding_func = loadstring('return ' .. sharding_func_def.body)
        if sharding_func == nil then
            return nil, string.format(
                "Body is incorrect in sharding_func for space (%s)", space_name)
        end
        return sharding_func()
    end
